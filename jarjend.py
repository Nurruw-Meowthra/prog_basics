...
def create_list_with_edge_elements(elements: list) -> list:
    ...
    """
    esielement = elements[0]
    viimaelement = elements[-1]
    return [esielement, viimaelement]
...
get_middle_part_of_list([1, 3, 6, 7])
...
create_new_list_with_added_number([20, 4, 9], 5)
...
def swap_edge_elements(elements: list) -> list:
    ...
    """
    esielement = elements[-1]
    viimaelement = elements[0]
    del elements[0]
    del elements[-1]
    elements.insert(0, esielement)
    elements.extend([viimaelement])
    return elements

swap_edge_elements([1, 2, 1, 4])
...
def get_repeated_list(elements: list, repetiton: int) -> list:
    ...
    pikendatud = []
    while repetiton > 0:
        pikendatud.extend(elements)
        repetiton -= 1
    return pikendatud

get_repeated_list([1], 5)
